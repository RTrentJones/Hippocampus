...
ARM_NAMES = ["semantic", "hybrid", "temporal", "causal", "anchor_causal"]
...
async def run_arms(
    ...
    return {
        "semantic": await arms.semantic_arm(case.query, provider, k),
        "hybrid": await arms.hybrid_arm(case.query, provider, k),
        "temporal": await arms.temporal_arm(anchor, k),
        "causal": await arms.causal_arm(anchor, k),
        "anchor_causal": await arms.anchor_causal_arm(case.query, provider, k),
    }
...
async def main_async(args: argparse.Namespace) -> dict:
    ...
    try:
        ...
        results = {case.session: await run_arms(case, provider, offsets, args.k) for case in cases}
